...
`npm test` uses built output, so build first. The suite covers protocol framing, structured controls, HTTP/WebSocket boundaries, ownership handoff, setup, diagnostics, and lifecycle helpers. Linux CI runs offline checks; it does not establish Linux host support or live Android compatibility. Windows phone/video checks are separate and require an authorized device.
